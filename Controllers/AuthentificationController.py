...
from utils.Token import generate_token, check_token
...
class AuthentificationController():
    ...
    def me(request: Request, token: str):
        ...
        ready_user = UserData(
            id = full_user['id'],
            nom = full_user['name'],
            tel = full_user['telephone'],
            raisonSociale = full_user['name_magasin'],
            adresse = full_user['street'],
            email = full_user['email'],
            idDetaillant = full_user['code'],
            niveauDetaillant = int(full_user['etoile'])-1,
            pointsDetaillant = full_user['nbr_points'],
            natureCommerce = full_user['categorie_id'][1] if full_user['categorie_id'] else None,
            ville = full_user['commune_id'][1] if full_user['commune_id'] else None,
            wilaya= full_user['state_id'][1] if full_user['state_id'] else None,
            pourcentageNiveau=80,
            pourcentageProfil=int((filled_fields / total_fields) * 100),
            socialMedia = ready_social_media,
            images= ready_images,
            otherTel = full_user['new_tlp1'],
    ...
    @staticmethod # Ready
    def login(request: Request, phone : str, password : str):
        odooDatabase : OdooDatabase = request.app.state.odooDatabase
        det = odooDatabase.execute_kw('info.cnx', 'search', [[['telephone', '=', phone]]])
        users = None
        if det:
